...
func ServeRequest[O any](plugin ServocPlugin[O]) error {
	...
	codec := jsonrpc.NewServerCodec(ipc.NewConn(os.Stdin, stdout))
	defer codec.Close()
	return srv.ServeRequest(codec)
}
...
func (pc *pluginCompat[O]) Generate(req *ipc.GenerateRequest, res *ipc.GenerateResponse) error {
	...
	for _, file := range req.Files {
		options := new(O)
		if err := parseOptions[O](TrimmedName(os.Args[0]), info.Options, file.Options, req.Options, options); err != nil {
