...
func MustParse(t *testing.T, content string) *ast.File {
	t.Helper()
	file, err := parser.File("file.srvo", strings.NewReader(TrimDedent(content)), nil)
	if err != nil {
...
func HasOption(t *testing.T, actual *ast.File, expected ast.Option) {
	t.Helper()
...
func HasOnlyOption(t *testing.T, actual *ast.File, expected ast.Option) {
	t.Helper()
