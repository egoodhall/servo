...
import (
	...
	"github.com/egoodhall/servo/parser/parsegen"
)

func Files(in []*os.File, options option.Set) ([]*ast.File, error) {
	files := make([]*ast.File, len(in))
	var err error
	for i, file := range in {
		files[i], err = File(file.Name(), file, options)
		if err != nil {
			return nil, err
		}
	}
...
func File(name string, in io.Reader, options option.Set) (*ast.File, error) {
	...
	f, err := gather(tc.tokens, options)
	if err != nil {
		return f, err
	}
...
func gather(tokens []token, options option.Set) (*ast.File, error) {
	file := new(ast.File)
	...
	for _, tkns := range tokenGroups {
		...
		case parsegen.OptionName:
			opt, err := gatherOption(tkns[0], tkns[1:], options)
			if err != nil {
				return nil, err
			}
			file.Options = append(file.Options, &ast.Option{
				Name:  opt.Name,
				Value: opt.Value,
			})
...
func gatherOption(name token, tokens []token, options option.Set) (*option.Option, error) {
	if len(tokens) < 1 {
		return nil, fmt.Errorf("token mismatch: need 1 or more values")
	}
	...
	if len(tokens) == 1 {
		if tok, err := parseOptionValue(tokens[0]); err != nil {
			return nil, err
		} else {
			value = tok
		}
		...
		case parsegen.OptionSlice:
			vals := make([]any, 0)
			for _, tok := range tokens[:len(tokens)-1] {
				if tok, err := parseOptionValue(tok); err != nil {
					return nil, err
				} else {
		...
		case parsegen.OptionMap:
			...
			for i, tok := range tokens[:len(tokens)-1] {
				if i%2 == 0 {
					...
					if ptok, err := parseOptionValue(tok); err != nil {
						return nil, err
					} else {
						vals[key] = ptok
					}
			...
			value = vals
		default:
			return nil, fmt.Errorf("unexpected option value token: %s", tokens[0].typ)
		}
	}
...
func parseOptionValue(tok token) (any, error) {
	var value any
	switch tok.typ {
	...
	case parsegen.OptionFloat:
		if v, err := strconv.ParseFloat(tok.value, 32); errors.Is(err, strconv.ErrRange) {
